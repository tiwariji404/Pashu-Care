  ...
  return (
    <>
      ...
      <div className="content">
        {(user?.role === 'gaushala_manager' || user?.role === 'admin') && tab === 'available' && !showAddForm && (
        ...
        {showAddForm && (
          <div className="card" style={{ marginBottom: '1.5rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              <h2 className="card-title" style={{ margin: 0 }}>Add Animal for Adoption</h2>
            ...
            <form onSubmit={handleAddSubmit}>
               ...
               <div className="form-group">
                 <label>Brief Description & Behavioral Notes</label>
                ...
                <div style={{ padding: '1.25rem' }}>
                  ...
                  {user?.role === 'user' ? (
                     <button className="btn btn-primary" style={{ width: '100%', display: 'flex', justifyContent: 'center', gap: '0.5rem' }} onClick={() => setShowApplyModal(animal.id)}>
                       <HeartHandshake size={18} /> Apply to Adopt / Foster
                     </button>
                  ) : (
                     <p style={{ margin: 0, fontSize: '0.85rem', color: 'var(--text-secondary)', textAlign: 'center' }}>Citizens can adopt this animal.</p>
                  )}
                </div>
              </div>
            )) : (
              <p style={{textAlign:'center', marginTop:'2rem', color:'var(--text-secondary)'}}>No animals are currently available for adoption.</p>
            )}
          </div>
        )}
          ...
          <div style={{ display: 'grid', gap: '1rem' }}>
            {myRequests.length > 0 ? myRequests.map(animal => (
              <div key={animal.id} className="card" style={{ borderLeft: animal.myRequestStatus === 'pending' ? '4px solid #f59e0b' : '4px solid var(--primary-color)' }}>
                ...
                <p style={{ margin: 0, fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Application sent for verification.</p>
                {animal.myRequestStatus === 'pending' && <p style={{ fontSize: '0.75rem', marginTop: '0.5rem', color: 'var(--text-secondary)' }}>Wait for the Shelter Manager to call you on {user?.phone}.</p>}
              </div>
            )) : (
              <p style={{textAlign:'center', marginTop:'2rem', color:'var(--text-secondary)'}}>You haven't made any adoption requests yet.</p>
            )}
          </div>
        )}
      </div>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
          <div className="card" style={{ width: '100%', maxWidth: '400px', margin: 0 }}>
            ...
            <form onSubmit={handleApplySubmit}>
              <div className="form-group">
                <label>Why do you want to adopt/foster?</label>
              ...
              <p style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginBottom: '1rem' }}>By applying, you agree to a background check and regular vet checkups.</p>
